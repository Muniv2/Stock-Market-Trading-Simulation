...
void Portfolio :: settleTradeAmounts (string buyerName, string sellerName , int tradePrice, int tradeQty) {
    ...
    if (buyerNode) {
        ...
        int newBudget = buyerNode->trader.getBudget() - tradeAmount;
        int newInventory = buyerNode->trader.getInventory() + tradeQty;
        // Portfolio value = remaining budget + (new shares * trade price )
        int newPortfolioValue = newBudget + (newInventory*tradePrice);
        updatePortfolio(buyerName, oldValue, newPortfolioValue, buyerNode->trader.getRole(), newBudget, newInventory, buyerNode->trader.getProfitLoss());
    }
    // Update seller : increase budget , decrease inventory
    OSTNode * sellerNode = findTrader(sellerName);
    if (sellerNode) {
        int oldValue = sellerNode->key;
        int newBudget = sellerNode->trader.getBudget() + tradeAmount;
        int newInventory = sellerNode->trader.getInventory() - tradeQty;
        int newPortfolioValue = newBudget + (newInventory * tradePrice);
        updatePortfolio (sellerName, oldValue, newPortfolioValue, sellerNode->trader.getRole(), newBudget, newInventory, sellerNode->trader.getProfitLoss());
    }
}
...
void Portfolio :: getRecentTrades (int n ) {
    ...
    for (int i = total; i > total - count; i--) {
        OSTNode * t = tradeHistoryOST.select(tradeHistoryOST.root, i);
        if (t) cout << "[ TRADE ] " << t -> trade.getBuyerName() << " <-> " << t -> trade.getSellerName() << " | $" << t -> trade . getPrice () << "\n";
    }
}
...
void Portfolio::displayLeaderboard() {
    int total = portfolioOST.totalNodes;
    if (total == 0) { cout << "\n[LEADERBOARD] No traders registered.\n"; return; }
    cout << "\n==========================================\n";
    cout << " TRADER LEADERBOARD \n";
    cout << "==========================================\n";
    cout << "Rank Name Role Portfolio\n";
    cout << "------------------------------------------\n";
    for (int i = total; i >= 1; i--) {
        OSTNode* node = portfolioOST.select(portfolioOST.root, i);
        if (node) {
            int displayRank = total - i + 1;
            cout << "#" << displayRank << " "
            << node->trader.getName() << "\t"
            << node->trader.getRole() << "\t$"
            << node->trader.getPortfolioValue() << "\n";
        }
    }
    cout << "==========================================\n";
}